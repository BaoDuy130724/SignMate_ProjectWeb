...
import { Plus, Edit2, Trash2, ChevronDown, ChevronRight, Video, List, Loader2, Building2 } from 'lucide-react';
import { coursesApi, lessonsApi } from '../services/api';

/**
 * Quản lý nội dung RIÊNG của trung tâm (CenterAdmin).
 * BE tự gán CenterId theo center của caller khi tạo, và chặn sửa/xóa nội dung ngoài center.
 * Trang này chỉ hiển thị khóa của chính center (lọc c.centerId) — không đụng khóa global của nền tảng.
 */
const CenterContent = () => {
  ...
  const fetchCourses = async () => {
    try {
      ...
      // Chỉ giữ khóa RIÊNG của center (centerId có giá trị); khóa global do SuperAdmin quản lý.
      setCourses((data || []).filter(c => c.centerId));
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleSaveCourse = async (e) => {
    e.preventDefault();
    try {
      if (editCourse) await coursesApi.update(editCourse.id, courseForm);
  ...
  const handleSaveLesson = async (e) => {
    e.preventDefault();
    try {
      if (editLesson) await lessonsApi.update(editLesson.id, lessonForm);
      else await lessonsApi.create(targetCourseId, lessonForm);
  ...
  const handleDeleteCourse = async (id) => {
    if (window.confirm('Xóa khóa học này và toàn bộ bài học bên trong?')) {
      try { await coursesApi.delete(id); fetchCourses(); }
  ...
  return (
    <>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 className="page-title">Nội dung Trung tâm</h1>
          <p className="page-subtitle">Tạo và quản lý khóa học, bài học riêng của trung tâm bạn. Chỉ học viên thuộc trung tâm mới học được.</p>
        </div>
      ...
      <div className="table-wrapper">
        <div className="table-header">
          <div className="table-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Building2 size={16} color="var(--primary)" /> Khóa học của Trung tâm
          </div>
        </div>
            ...
            {courses.length === 0 && (
              <tr>
                <td colSpan="6" style={{ textAlign: 'center', padding: '48px', color: 'var(--gray-400)' }}>
                  Trung tâm chưa có khóa học riêng nào. Bấm "Tạo Khóa học mới" để bắt đầu.
                </td>
              </tr>
            )}
          </tbody>
            ...
            <form onSubmit={handleSaveCourse}>
              ...
              <div className="grid-2">
                ...
                <div className="form-group" style={{ gridColumn: 'span 2', display: 'flex', alignItems: 'center', gap: '10px', marginTop: '10px' }}>
                  <input type="checkbox" id="cc_isPublished" checked={courseForm.isPublished} onChange={e => setCourseForm({...courseForm, isPublished: e.target.checked})} style={{ width: '18px', height: '18px', cursor: 'pointer' }} />
                  <label htmlFor="cc_isPublished" style={{ fontWeight: 700, cursor: 'pointer', color: 'var(--primary)' }}>Công khai cho học viên trung tâm (Public)</label>
                </div>
              </div>
      ...
      {showLessonModal && (
        <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="card" style={{ width: '500px', animation: 'slideIn 0.3s ease' }}>
            <h2 style={{ marginBottom: '20px' }}>{editLesson ? 'Cập nhật Bài học' : 'Thêm Bài học mới'}</h2>
            <form onSubmit={handleSaveLesson}>
              ...
              <div className="grid-2">
                <div className="form-group">
                  ...
                  <input type="number" className="form-input" value={lessonForm.durationSeconds} onChange={e => setLessonForm({...lessonForm, durationSeconds: parseInt(e.target.value) || 0})} />
                </div>
                <div className="form-group">
                  <label className="form-label">Thứ tự hiển thị</label>
                  <input type="number" className="form-input" value={lessonForm.orderIndex} onChange={e => setLessonForm({...lessonForm, orderIndex: parseInt(e.target.value) || 0})} />
                </div>
              </div>
              <div style={{ display: 'flex', gap: '12px', marginTop: '20px' }}>
